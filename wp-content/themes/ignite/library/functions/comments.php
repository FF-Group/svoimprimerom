...
function hybrid_comment_form_args( $args ) {
	...
	$args = array(
		...
		'must_log_in'          => '<p class="alert">' . sprintf( __( 'Вы должны <a href="%1$s" title="Log in">войти</a> для комментирования.', 'hybrid-core' ), wp_login_url( get_permalink() ) ) . '</p><!-- .alert -->',
		'logged_in_as'         => '<p class="log-in-out">' . sprintf( __( 'Вы вошли как <a href="%1$s" title="%2$s">%2$s</a>.', 'hybrid-core' ), admin_url( 'profile.php' ), esc_attr( $user_identity ) ) . ' <a href="' . wp_logout_url( get_permalink() ) . '" title="' . esc_attr__( 'Выйти', 'hybrid-core' ) . '">' . __( 'Выйти &raquo;', 'hybrid-core' ) . '</a></p><!-- .log-in-out -->',
		'comment_notes_before' => '',
		'comment_notes_after'  => '',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Комментировать', 'hybrid-core' ),
		'title_reply_to'       => __( 'Комментировать %s', 'hybrid-core' ),
		'cancel_reply_link'    => __( 'Отмена', 'hybrid-core' ),
		'label_submit'         => __( 'Оставить комментарий', 'hybrid-core' ),
	);
